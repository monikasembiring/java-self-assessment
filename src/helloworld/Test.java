...
public class Test {
	public static void main (String[] args) {
		...
		while (complete1 == false && complete2 == false) {
			...
			hitShip1(board2, historyBoard1, row, col);
			printBattleShip(historyBoard1);
			complete1 = finalState(board2);
			if (complete1 == true) {
				winner = 1;
				break;
			} else {
				...
				hitShip2(board1, historyBoard2, row, col);
				printBattleShip(historyBoard2);
				complete2 = finalState(board1);
				if (complete2 == true) {
					winner = 2;
					break;
				}
		...
		System.out.println("");
		System.out.println("Player 1 boards:");
		printBattleShip(board1);
		System.out.println("");
		System.out.println("Player 2 boards:");
		printBattleShip(board2);
	}
	...
	private static void hitShip1(char[][] board, char[][] history, int row, int col) {
		if (board[row][col] == '@') {
			history[row][col] = 'X';
			board[row][col] = 'X';
	...
	private static void hitShip2(char[][] board, char[][] history, int row, int col) {
		if (board[row][col] == '@') {
			history[row][col] = 'X';
			board[row][col] = 'X';
